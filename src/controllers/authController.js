...
class AuthController {
    ...
    try {
      ...
      if (!["email", "contact"].includes(type)) {
        return errorResponse(
          res,
          'Invalid OTP type. Must be "email" or "contact"',
          400
        );
      }
  ...
  async initiateParentConsent(req, res) {
    try {
      ...
      const result = await authService.initiateParentConsent(
        tempToken,
        parentEmail,
        parentContact
      );
  ...
  async resendParentOTP(req, res) {
    try {
      ...
      return successResponse(
        res,
        null,
        `Parent ${type} OTP resent successfully`
      );
    } catch (error) {
  ...
  async invitePsychologist(req, res) {
    try {
      ...
      return successResponse(
        res,
        { email },
        "Psychologist invitation sent successfully"
      );
    } catch (error) {
  ...
  async verifyPsychologistInvite(req, res) {
    try {
      ...
      const psychologist = await authService.createPsychologistUser(
        invite.email,
        password,
        psychologistData
      );
