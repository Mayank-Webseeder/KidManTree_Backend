  ...
  async adminSetUserActiveStatus(req, res) {
    try {
      ...
      if (user.role === 'psychologist') {
        await Psychologist.findOneAndUpdate({ email: user.email }, { isActive });
      }
  ...
  async adminUpdateUserProfile(req, res) {
    try {
      ...
      if (profile !== undefined) {
        ...
        if (profile.emergencyContact !== undefined) updateData['profile.emergencyContact'] = profile.emergencyContact;
      }
      ...
      const message = error.code === 11000 ? 'Email or contact already exists' : 'Failed to update user';
